...
class MRBLaT_Functions():
    ...
    def __init__(self, radar_parameters): 
        ...
        self.__A = np.sqrt(self.__transmitPower * self.__gain * self.__wavelength**2 / (4 * np.pi)**3)
    ...
    def jacobian_S_A(self, epsilon, print_output=False):
        ...
        if print_output:
            print(f"dAdx  : {(np.abs(dAdx))}     -     dAdx shape  : {dAdx.shape}")
            print(f"expfx : {(np.abs(expfx))}     -     expfx shape : {expfx.shape}")
            print(f"kernal: {(np.abs(kernal))}     -     kernal shape: {kernal.shape}")
            print(f"A     : {(np.abs(A))}     -     A shape      : {A.shape}")
            print(f"dexpdx: {(np.abs(dexpdx))}     -     dexpdx shape : {dexpdx.shape}")
            print(f"dsindx : {(np.abs(dsindx))}     -     dsindx shape : {dsindx.shape}")
    ...
    def jacobian_S_H(self, epsilon, print_output=False):
        ...
        Jy = dAdy*(expfx*kernal) + A*(dexpdy*kernal + expfx*dsindy)
        if print_output:
            print(f"phi   : {(np.abs(phi))}     -     phi shape   : {phi.shape}")
            # print(f"dAdx  : {(np.abs(dAdx))}     -     dAdx shape  : {dAdx.shape}")
            # print(f"expfx : {(np.abs(expfx))}     -     expfx shape : {expfx.shape}")
            # print(f"kernal: {(np.abs(kernal))}     -     kernal shape: {kernal.shape}")
            # print(f"A     : {(np.abs(A))}     -     A shape      : {A.shape}")
    ...
    def D_KL(self, params, Z_data, phi_last, inputmode=(1,1,1,1), outputmode=(1,1,1,1), print_output=False):
        ...
        term_3 = (np.abs(alpha_hat_xy)**2 * (term_3_inner_prod[0,0]*eps_barbar_0 + term_3_inner_prod[1,1]* eps_barbar_1))/((self.__standardDeviation*np.sqrt(self.__N_samples))**2) #np.trace(np.array([[eps_barbar_0, 0], [0, eps_barbar_1]]) @ term_3_inner_prod)
        # print(f"X : {eps_bar_x} Y: {eps_bar_y} E0: {(eps_barbar_0)} E1: {(eps_barbar_1)} - term_3: {(term_3_inner_prod[0,0]*eps_barbar_0 + term_3_inner_prod[1,1]* eps_barbar_1)}")
        # print(f"T: {np.abs(S_N_lack.conj().T@Z_data)} N: {np.abs(S_N_lack.conj().T@S_N_lack)} - ALPHA: {alpha_hat_xy}")
        # print(f"ENTROPY: {2/2 * np.log(2*np.pi*np.exp(1)) + 1/2*np.log(eps_barbar_0 * eps_barbar_1)}")
        # dddddddddd
        ...
        return np.real(outputmode[0]*term_1 + outputmode[1]*term_2 + outputmode[2]*term_3 - outputmode[3]*entropy)
